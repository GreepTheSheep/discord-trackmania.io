            ...
            sql.query("SELECT * FROM `players` WHERE discordId = ?", message.author.id, (err, res) =>{
                if (err){
                    ...
                    players.player(res[0].accountId).then(player=>{
                        ...
                        .setDescription(player.displayName + ' has started playing Trackmania ' + ms(new Date(player.timestamp), {compact: true, verbose: true}) + ' ago.\nThis player was last seen ' + ms(new Date(player.trophies.timestamp), {compact: true, verbose: true}) + ' ago.\nIts Trackmania.io URL is ' + player.url)
