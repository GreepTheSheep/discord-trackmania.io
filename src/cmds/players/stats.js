            ...
            sql.query("SELECT * FROM `players` WHERE discordId = ?", message.author.id, (err, res) =>{
                if (err){
                    ...
                    players.player(res[0].accountId).then(player=>{
                        ...
                        .setDescription(player.displayName + ' has started playing Trackmania ' + ms(new Date().getMilliseconds() - new Date(player.timestamp).getMilliseconds(), {compact: true, verbose: true}) + ' ago.\nThis player was last seen ' + ms(new Date().getMilliseconds() - new Date(player.trophies.timestamp).getMilliseconds(), {compact: true, verbose: true}) + ' ago.\nIts Trackmania.io URL is ' + player.url)
