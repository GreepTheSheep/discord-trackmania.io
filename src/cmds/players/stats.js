            ...
            sql.query("SELECT * FROM `players` WHERE discordId = ?", message.author.id, (err, res) =>{
                if (err){
                    ...
                    players.player(res[0].accountId).then(player=>{
                        ...
                        .setDescription(player.displayname + ' has started playing Trackmania ' + ms(new Date(player.timestamp).getMilliseconds(), {verbose: true}) + ' ago.\nThis player was last seen ' + ms(new Date(player.trophies.timestamp).getMilliseconds(), {verbose: true}) + ' ago.\nIts Trackmania.io URL is ' + player.url)
