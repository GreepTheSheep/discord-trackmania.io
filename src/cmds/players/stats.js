        ...
        if (args.length < 1) {
            ...
            players.player(args[0]).then(player=>{
                var trophies_str = []
                    ...
                    .addField('Ranking:', `${player.trophies.points} points (${player.trophies.echelonName})\n__Number of trophies__:\n${trophies_str.join('\n')}`, true)
                    .addField('Top:', top_str.join('\n'), true)
                    if (player.matchmaking.length > 0) embed.addField('Matchmaking:', `**__Teams 3v3:__**\nScore: ${player.matchmaking.find(m=>m.info.typename == '3v3').info.score}/${player.matchmaking.find(m=>m.info.typename == '3v3').info.rank.before} (${player.matchmaking.find(m=>m.info.typename == '3v3').info.rank.name})\nRank: ${player.matchmaking.find(m=>m.info.typename == '3v3').info.place}/${player.matchmaking.find(m=>m.info.typename == '3v3').total}`)
